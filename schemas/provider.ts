import { z } from "zod";
export const providerTypes=z.enum(["openai","anthropic","google-gemini","openrouter","groq","mistral","cohere","deepseek","xai","azure-openai","amazon-bedrock","ollama","openai-compatible"]);
export const providerAuthTypes=z.enum(["bearer","api_key_header","query","basic","custom_headers","oauth_client_credentials","none"]);
export const providerInputSchema=z.object({name:z.string().trim().min(2).max(100),slug:z.string().regex(/^[a-z0-9][a-z0-9-]{1,62}$/),providerType:providerTypes,baseUrl:z.url(),authType:providerAuthTypes.default("bearer"),chatEndpoint:z.string().max(200).default("chat/completions"),modelsEndpoint:z.string().max(200).default("models"),apiVersion:z.string().max(50).optional(),timeoutMs:z.number().int().min(1000).max(600000).default(90000),retryCount:z.number().int().min(0).max(5).default(2),priority:z.number().int().min(0).max(10000).default(100),enabled:z.boolean().default(false),configuration:z.record(z.string(),z.unknown()).default({}),credential:z.object({secret:z.string().min(4).max(20_000),headerName:z.string().max(100).optional(),username:z.string().max(200).optional(),isDefault:z.boolean().default(true)}).optional()});
