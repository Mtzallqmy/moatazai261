...
export type SecretContext=
 ...
 |{mcpServerId:string}
 |{providerId:string;userId:string;userApiKeyId:string};
...
export class AesGcmSecretCrypto implements SecretCrypto{
 ...
 private aad(c:SecretContext){if("mcpServerId" in c)return encoder.encode(`mcp:${c.mcpServerId}:v1`);if("userApiKeyId" in c)return encoder.encode(`provider:${c.providerId}:user:${c.userId}:key:${c.userApiKeyId}:v1`);return encoder.encode(`provider:${c.providerId}:credential:${c.credentialId}:v1`);}
